...
const generateCornerTrees = (
  ...
  // Diagonal branches from top edge — 50% of count
  ...
  // Vertical branches hanging straight down from top — adds density
  const vertCount = Math.ceil(count * 0.35);
  for (let i = 0; i < vertCount; i++) {
    const t = i / Math.max(vertCount - 1, 1);
    // Spread across the top edge
    const rawX = 0.95 - t * 0.75 + (rand() - 0.5) * 0.1;
    const x = isRight ? rawX : 1 - rawX;
    const y = -0.02 - rand() * 0.02;

    // Mostly vertical (pointing straight down) with slight variation
  ...
  // Some from the side edge (upper portion) — rest
...
const generateBokeh = (count: number, baseSeed: number): BokehDef[] => {
  const rand = createRNG(baseSeed);
  const bokeh: BokehDef[] = [];
...
const drawBokeh = (
  ...
  for (const b of bokeh) {
    const x = (b.x + Math.sin(time * 0.15 + b.phase) * b.driftX) * w;
    const y = (b.y + Math.cos(time * 0.12 + b.phase * 1.3) * b.driftY) * h;
    const r = b.radius * (1 + Math.sin(time * 0.2 + b.phase) * 0.08);

    const grad = ctx.createRadialGradient(x, y, 0, x, y, r);
    const a = b.opacity * opacity;
    grad.addColorStop(0, `rgba(255,255,255,${a})`);
    grad.addColorStop(0.5, `rgba(255,255,255,${a * 0.5})`);
    grad.addColorStop(0.8, `rgba(255,255,255,${a * 0.15})`);
...
// ── Drawing: Filled branch silhouette ───────────────────────────────
...
const drawBranchSilhouette = (
  ...
  // Draw as filled shape — left edge forward, tip point, right edge backward
  ...
  // Converge at tip center point (clean taper, no arc)
  ...
  // Endpoint angle for child branches
  ...
  // Leaves at branch tips — small scattered silhouettes
...
const paint = (
  ...
  const farTrees = trees.filter(t => t.depthLayer === 0);
  const midTrees = trees.filter(t => t.depthLayer === 1);
  const nearTrees = trees.filter(t => t.depthLayer === 2);

  // FAR layer
  applyPass(ctx, 40, 0.06 * o, (c) => {
    paintScene(c, w, h, farTrees, time, windStrength, windSpeed);
  });
  applyPass(ctx, 24, 0.05 * o, (c) => {
    paintScene(c, w, h, farTrees, time, windStrength, windSpeed);
  });

  // MID layer
  applyPass(ctx, 28, 0.05 * o, (c) => {
    paintScene(c, w, h, midTrees, time, windStrength, windSpeed);
  });
  applyPass(ctx, 14, 0.08 * o, (c) => {
    paintScene(c, w, h, midTrees, time, windStrength, windSpeed);
  });

  // Bokeh
  drawBokeh(ctx, w, h, bokeh, time, bokehOpacity);

  applyPass(ctx, 8, 0.06 * o, (c) => {
    paintScene(c, w, h, midTrees, time, windStrength, windSpeed);
  });

  // NEAR layer
  applyPass(ctx, 18, 0.05 * o, (c) => {
    paintScene(c, w, h, nearTrees, time, windStrength, windSpeed);
  });
  applyPass(ctx, 10, 0.08 * o, (c) => {
    paintScene(c, w, h, nearTrees, time, windStrength, windSpeed);
  });
  applyPass(ctx, 6, 0.06 * o, (c) => {
    paintScene(c, w, h, nearTrees, time, windStrength, windSpeed);
  });

  // Top bokeh
  drawBokeh(ctx, w, h, bokeh.slice(0, Math.floor(bokeh.length * 0.3)), time + 100, bokehOpacity * 0.5);
};
...
const BackgroundTree = ({
  ...
  const propsRef = useRef({ transparent, backgroundColor, shadowOpacity, windStrength, windSpeed, bokehOpacity });
  propsRef.current = { transparent, backgroundColor, shadowOpacity, windStrength, windSpeed, bokehOpacity };

  const { trees, bokeh } = useMemo(() => {
    const allTrees = generateCornerTrees(branchCount, 500, position, branchWidth);
    const allBokeh = generateBokeh(bokehCount, 777);
    return { trees: allTrees, bokeh: allBokeh };
  }, [branchCount, branchWidth, bokehCount, position]);
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number): void => {
      ...
      paint(ctx, w, h, p.transparent, p.backgroundColor, trees, bokeh, time, p.windStrength, p.windSpeed, p.shadowOpacity, p.bokehOpacity);
